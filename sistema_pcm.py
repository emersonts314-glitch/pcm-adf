...
# --- CARREGAMENTO DE DADOS (VIA API HTTP DIRETA) ---
def carregar_dados():
    colunas = [
    ...
    try:
        ...
        # Garante as colunas e formatações
        for col in colunas:
        ...
        st.error(f"⚠️ Erro ao conectar com o banco de dados na nuvem: {e}")
        return pd.DataFrame(columns=colunas)

# --- SALVAR DADOS (VIA API HTTP DIRETA) ---
def salvar_dados(df_to_save):
    try:
        df_clean = df_to_save.copy()
        
        def date_to_str(val):
            if pd.isna(val) or val == "" or str(val).lower() == 'nat': return None
            if isinstance(val, (date, datetime)): return val.strftime('%d/%m/%Y')
            return str(val)

        for col in ['Data_Emissao', 'Data_Inicio', 'Data_Fim']:
            df_clean[col] = df_clean[col].apply(date_to_str)
            
        # O Banco prefere "None" no lugar de vazios para não quebrar colunas de números/datas
        ...
        headers = {
            ...
            "Authorization": f"Bearer {SUPABASE_KEY}",
            "Content-Type": "application/json",
            "Prefer": "resolution=merge-duplicates" # Comando mágico para UPSERT (Atualiza se existir)
        }
        ...
        st.error(f"⚠️ Erro ao salvar dados na nuvem: {e}")
